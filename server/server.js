...
app.use(
  cors({
    origin: ["http://78.188.217.104:3000", "http://78.188.217.104:2431"],
    credentials: true,
...
app.use((req, res, next) => {
  res.header("Access-Control-Allow-Origin", "http://78.188.217.104:3000"); // İstemci adresi
...
function authMiddleware(req, res, next) {
  ...
  if (clientIP !== "http://78.188.217.104:3000") {
    return res.redirect("http://78.188.217.104:3000/");
  }
...
app.get("/api-client/qr-sorgula", (req, res) => {
  ...
  if (!fishNo) {
    return res.redirect(`http://78.188.217.104:3000/`); // ❌ Eğer fishNo yoksa anasayfaya yönlendir
  }
  ...
  db.query(query, (err, results) => {
    ...
    if (results.length > 0) {
      ...
      res.redirect(
        `http://78.188.217.104:3000/client?record=${encodeURIComponent(
          userRecord
        )}`
      );
    } else {
      res.redirect(`http://78.188.217.104:3000/`); // ❌ Kullanıcı yoksa anasayfaya yönlendir
    }
  });
});
