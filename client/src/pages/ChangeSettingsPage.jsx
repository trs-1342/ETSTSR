...
export default function ChangeSettingsPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get("http://localhost:2431/api/get-users-data");
  ...
  useEffect(() => {
    ...
    const fetchUserPermissions = async () => {
      try {
        const response = await axios.get(`http://localhost:2431/api/get-user-permissions/${selectedUser}`);
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:2431/api/change-user-settings", {
