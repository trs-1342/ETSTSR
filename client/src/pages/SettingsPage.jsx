...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("http://192.168.1.108:2431/api/get-users-data");
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await fetch("http://192.168.1.108:2431/api/checkAdmin", {
          credentials: "include",
        });
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      const response = await fetch(`http://192.168.1.108:2431/api/delete-user/${userId}`, {
        method: "DELETE",
      });
